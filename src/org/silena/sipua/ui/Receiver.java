...
package org.silena.sipua.ui;
import org.silena.R;
import org.silena.sipua.SipdroidEngine;
import org.silena.sipua.UserAgent;
...
import org.silena.media.Bluetooth;
import org.silena.media.RtpStreamReceiver;
import org.silena.media.RtpStreamSender;
import org.silena.sipua.phone.Call;
import org.silena.sipua.phone.Connection;
	...
	public class Receiver extends BroadcastReceiver {
			...
			if (call_state != state) {
				...
				case UserAgent.UA_STATE_INCOMING_CALL:
					...
					if ((pstn_state == null || pstn_state.equals("IDLE")) &&
							PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_AUTO_ON, org.silena.sipua.ui.Settings.DEFAULT_AUTO_ON) &&
						...
						if (am.getStreamVolume(AudioManager.STREAM_RING) > 0) {				 
							String sUriSipRingtone = PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.silena.sipua.ui.Settings.PREF_SIPRINGTONE,
		...
		public static void onText(int type,String text,int mInCallResId,long base) {
			...
			if (type >= REGISTER_NOTIFICATION && mInCallResId == R.drawable.sym_presence_available &&
					!PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_REGISTRATION, org.silena.sipua.ui.Settings.DEFAULT_REGISTRATION))
				text = null;
	        ...
	        if (text != null) {
				...
				if (type == MISSED_CALL_NOTIFICATION) {
			        	...
			        	if (PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_NOTIFY, org.silena.sipua.ui.Settings.DEFAULT_NOTIFY)) {
					...
					if (base != 0) {
						...
						if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_POS, org.silena.sipua.ui.Settings.DEFAULT_POS))
		...
		static void updateAutoAnswer() {
			if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_AUTO_ONDEMAND, org.silena.sipua.ui.Settings.DEFAULT_AUTO_ONDEMAND) &&
				Sipdroid.on(mContext)) {
				if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_AUTO_DEMAND, org.silena.sipua.ui.Settings.DEFAULT_AUTO_DEMAND))
					updateAutoAnswer(1);
				else
					updateAutoAnswer(0);
			} else
		...
		public static void pos(boolean enable) {
			...
			if (!PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_POS, org.silena.sipua.ui.Settings.DEFAULT_POS) ||
			PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.silena.sipua.ui.Settings.PREF_POSURL, org.silena.sipua.ui.Settings.DEFAULT_POSURL).length() < 1) {
				if (lm != null && am != null) {
					pos_gps(false);
					pos_net(false);
					lm = null;
			...
			if (enable) {
				if (call_state == UserAgent.UA_STATE_IDLE && Sipdroid.on(mContext) &&
						PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_POS, org.silena.sipua.ui.Settings.DEFAULT_POS) &&
						PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.silena.sipua.ui.Settings.PREF_POSURL, org.silena.sipua.ui.Settings.DEFAULT_POSURL).length()>0) {
		...
		static void enable_wifi(boolean enable) {
			if (!PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_OWNWIFI, org.silena.sipua.ui.Settings.DEFAULT_OWNWIFI))
				return;
			if (enable && !PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_WIFI_DISABLED, org.silena.sipua.ui.Settings.DEFAULT_WIFI_DISABLED))
        		return;
    		...
    		edit.putBoolean(org.silena.sipua.ui.Settings.PREF_WIFI_DISABLED,!enable);
    		edit.commit();
    		/*
    		if (enable) {
		...
		public static void url(final String opt) {
	        (new Thread() {
				public void run() {
					try {
				        URL url = new URL(PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.silena.sipua.ui.Settings.PREF_POSURL, org.silena.sipua.ui.Settings.DEFAULT_POSURL)+
				        		"?"+opt);
				        BufferedReader in;
		...
		static boolean on_vpn() {
			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_ON_VPN, org.silena.sipua.ui.Settings.DEFAULT_ON_VPN);
		}
		
		static void on_vpn(boolean enable) {
    		Editor edit = PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).edit();
    		
    		edit.putBoolean(org.silena.sipua.ui.Settings.PREF_ON_VPN,enable);
    		edit.commit();
		}
		...
		public static boolean isFast(int i) {
       	...
       	if (PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.silena.sipua.ui.Settings.PREF_USERNAME+(i!=0?i:""),"").equals("") ||
       			PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.silena.sipua.ui.Settings.PREF_SERVER+(i!=0?i:""),"").equals(""))
        		return false;
        	if (wi != null) {
	        	...
	        	if (wi.getIpAddress() != 0 && (WifiInfo.getDetailedStateOf(wi.getSupplicantState()) == DetailedState.OBTAINING_IPADDR
	        		...
	        		on_wlan = true;
	        		if (!on_vpn())
	        			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_WLAN+(i!=0?i:""), org.silena.sipua.ui.Settings.DEFAULT_WLAN);
	        		else
	        			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_VPN+(i!=0?i:""), org.silena.sipua.ui.Settings.DEFAULT_VPN);  
	        	}
        	}
        	on_wlan = false;
			return isFastGSM(i);
		...
		static boolean isFastGSM(int i) {
        	...
        	if (tm.getNetworkType() >= TelephonyManager.NETWORK_TYPE_UMTS)
        		return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_3G+(i!=0?i:""), org.silena.sipua.ui.Settings.DEFAULT_3G);
        	if (tm.getNetworkType() == TelephonyManager.NETWORK_TYPE_EDGE)
       			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_EDGE+(i!=0?i:""), org.silena.sipua.ui.Settings.DEFAULT_EDGE);
        	return false;
		}
		...
		public void onReceive(Context context, Intent intent) {
		    ...
		    if (intentAction.equals(WifiManager.WIFI_STATE_CHANGED_ACTION)) {
				if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_SELECTWIFI, org.silena.sipua.ui.Settings.DEFAULT_SELECTWIFI))
	        ...
	        if (intentAction.equals(WifiManager.SCAN_RESULTS_AVAILABLE_ACTION)) {
	        	if (SystemClock.uptimeMillis() > lastscan + 45000 &&
	        			PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.silena.sipua.ui.Settings.PREF_SELECTWIFI, org.silena.sipua.ui.Settings.DEFAULT_SELECTWIFI)) {
		        	WifiManager wm = (WifiManager) mContext.getSystemService(Context.WIFI_SERVICE);
		        	WifiInfo wi = wm.getConnectionInfo();
